...
def save_race_prediction(
    ...
    INSERT INTO race_records
      (race_id, race_name, race_date, venue, surface, distance, track_condition,
       predicted_at, confidence_top_score, top_horse, bias_type, pace_predicted, budget, note)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (race_id, race_name, race_date, venue, surface, distance, track_condition,
...
def save_result_to_diary(race_record_id: int, fetched_data: dict, bets: list[dict]):
    ...
    # ヒット判定の更新（簡易版：着順1〜3位に買い馬が含まれるか）
    top3 = {r["horse_name"] for r in fetched_data.get("results", []) if r["rank"] <= 3}
    c.execute("SELECT id, horses, amount FROM bets WHERE race_record_id=?", (race_record_id,))
    for bet_id, horses_json, amount in c.fetchall():
        horses = json.loads(horses_json)
        hit = all(h in top3 for h in horses if h != "残り全頭")
        payout = 0
        if hit:
            # 払い戻し金額を推定（実際には買い目タイプで異なる）
            first_payout = next(iter(fetched_data.get("payouts", {}).values()), {})
            payout = first_payout.get("amount", 0) * (amount // 100)
